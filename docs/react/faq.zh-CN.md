...
## `undefined` 和 `null` 在 `antd` 的受控组件中有区别吗？ {#difference-between-undefined-and-null}
...
## 官方文档中没有提供的隐藏 API 我可以使用吗？ {#internal-api}

不推荐。对内接口不保证兼容性，它很可能在某个版本中因重构而移除。如果你确实需要使用，需自行确保版本升级时隐藏接口仍旧可用，或者锁定版本。

## 为何新增 API 请求需要严格讨论？ {#why-strict-api-discussion}

我们在添加 API 时十分谨慎，因为一些 API 可能不够抽象从而变成历史债务。例如当需要对交互方式进行更改，这些不良抽象可能会引发 Breaking Change。为了避免诸如此类问题，我们推荐新功能优先通过 HOC 实现。

## 当我点击 `Select Dropdown DatePicker TimePicker Popover Popconfirm` 内的另一个 popup 组件时它会消失，如何解决？ {#nested-popup-disappear}
...
## `Select Dropdown DatePicker TimePicker Popover Popconfirm` 会跟随滚动条上下移动？ {#prevent-popup-scroll}
...
## 如何修改 Ant Design 的默认主题？ {#how-to-modify-default-theme}

可以参考[定制主题](/docs/react/customize-theme-cn)。

## 如何修改 Ant Design 组件的默认样式？ {#how-to-modify-component-style}

你可以覆盖它们的样式，但是我们不推荐这么做。antd 是一系列 React 组件，但同样是一套设计规范。

## 如何避免升级导致的破坏性变更？ {#how-to-avoid-breaking-change}
...
## 如何使用其他时间日期库如 Moment.js？ {#how-to-use-custom-date-lib}

可以参考[使用自定义日期库](/docs/react/use-custom-date-library-cn)。

## 当我动态改变 `defaultValue` 的时候它并没有生效。 {#dynamic-defaultValue-not-working}
...
## 为什么修改组件传入的对象或数组属性组件不会更新？ {#modifying-mutable-props-does-not-update}

antd 内部会对 props 进行浅比较实现性能优化。当状态变更，你总是应该传递一个新的对象。具体请参考 [React 的文档](https://zh-hans.reactjs.org/docs/thinking-in-react.html)

## 当我设置了 `Input`/`Select` 等的 `value` 时它就无法修改了。 {#cannot-change-value-after-setting}
...
## 多个组件放一排时没有垂直对齐怎么办？ {#components-not-vertically-aligned}

尝试使用 [Space](/components/space-cn) 组件来使他们对齐。

## antd 覆盖了我的全局样式！ {#global-styles-override}
...
## 我没法安装 `antd` 和 `antd` 的依赖，顺便提一句，我在中国大陆。 {#install-in-china}

那啥，试试 [npmmirror 国内镜像](https://npmmirror.com) 和 [cnpm](https://github.com/cnpm/cnpm)。

## 我在 package.json 里将 `dependencies.antd` 添加到了 git repository 中，但是没有用。 {#git-repository-dependency-not-working}

当然没用了，请使用 npm 安装 `antd`。

## `message` 和 `notification` 是小写的，但是其他的组件都是首字母大写的，这是手滑吗？ {#why-message-notification-lowercase}

不，因为 `message` 是一个函数，而不是一个 React 组件。

## `antd` 在移动端体验不佳。 {#mobile-support}
...
## `antd` 可以像 `React` 那样使用单文件引入吗？ {#standalone-files}
...
## 在我的网络环境下没法获取到 `icon` 文件。 {#icon-file-network}
...
## 如何拓展 antd 的组件？ {#how-to-extend-components}
...
## 如何获取未导出的属性定义？ {#how-to-get-unexported-definitions}
...
## 我的组件默认语言是英文的？如何切回中文的。 {#default-locale}
...
## 为什么时间类组件的国际化 locale 设置不生效？ {#date-component-locale-not-working}
...
## 开启了 Content Security Policy (CSP) 如何处理动态样式？ {#handling-dynamic-styles-with-csp}

你可以通过 [ConfigProvider](/components/config-provider-cn#content-security-policy) 来配置 `nonce` 属性。

## 当我指定了 DatePicker/RangePicker 的 `mode` 属性后，点击后无法选择年份/月份？ {#cannot-select-date-with-mode-set}
...
## ConfigProvider 设置 `prefixCls` 后，message/notification/Modal.confirm 生成的节点样式丢失了？ {#static-methods-style-lost-with-prefixCls}
...
## 为什么我不应该通过 ref 访问组件内部的 props 和 state？ {#why-not-access-internal-props-state-via-ref}
...
## 弹层类组件为什么要统一至 `open` 属性？ {#why-open-prop}
...
## 如何支持 SSR？ {#ssr-support}

请参考动态主题文档 [服务端渲染](/docs/react/server-side-rendering-cn) 部分内容。

## V5 中 colorPrimary 和 colorInfo 及 colorLink 之间是什么关系？ {#relationship-between-colorPrimary-colorInfo-colorLink}
...
## 如何正确的拼写 Ant Design？ {#spell-ant-design}
...
## 你们有接受捐助的渠道吗，比如支付宝或者微信支付？ {#monetary-donation-channels}

[https://opencollective.com/ant-design](https://opencollective.com/ant-design)

## 使用表单组件的 `setFieldsValue` 方法如果对象类型中含有 `null` 时 TS 类型报错 {#typescript-error-when-setFieldsValue-with-null}
...
## 使用 Next.js 的 App Router 时 antd 组件报错 {#nextjs-app-router-error}
