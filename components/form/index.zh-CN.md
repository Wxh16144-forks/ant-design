...
### 为什么 `getFieldsValue` 在初次渲染的时候拿不到值？ {#faq-get-fields-value-first-render}

`getFieldsValue` 默认返回收集的字段数据，而在初次渲染时 Form.Item 节点尚未渲染，因而无法收集到数据。你可以通过 `getFieldsValue(true)` 来获取所有字段数据。

### 为什么 `setFieldsValue` 设置字段为 `undefined` 时，有的组件不会重置为空？ {#faq-set-fields-value-undefined}
...
### 为什么字段设置 `rules` 后更改值 `onFieldsChange` 会触发三次？ {#faq-on-fields-change-trigger-three-times}
...
### 为什么 Form.List 不支持 `label` 还需要使用 ErrorList 展示错误？ {#faq-form-list-label-error-list}
...
### `scrollToFirstError` 和 `scrollToField` 失效？ {#faq-scroll-to-field-failed}
...
### 为什么表单点击 label 会更改组件状态？ {#faq-label-click-state-change}
